...
void blobStreamOutInit(BlobStreamOut *self, ImprintAllocator *allocator,
                       ImprintAllocatorWithFree *blobAllocator,
                       const uint8_t *data, size_t octetCount,
  ...
  self->entries =
      IMPRINT_ALLOC_TYPE_COUNT(allocator, BlobStreamOutEntry, self->chunkCount);
  ...
  CLOG_C_VERBOSE(&self->log,
      "blobStreamOutInit octetCount: %zu chunkCount: %zu fixedChunkSize %zu",
      octetCount, self->chunkCount, self->fixedChunkSize)
}
...
void blobStreamOutMarkReceived(BlobStreamOut *self,
  ...
  CLOG_C_VERBOSE(&self->log, "markReceived remote expecting %04X mask %08X",
...
int blobStreamOutGetChunksToSend(BlobStreamOut *self, MonotonicTimeMs now,
  ...
  for (size_t i = 0; i < self->chunkCount; ++i) {
    BlobStreamOutEntry *entry = &self->entries[i];
    if (!entry->isReceived && (((now - entry->lastSentAtTime > threshold) &&
                                entry->octetCount != 0) ||
                               entry->sendCount == 0)) {
