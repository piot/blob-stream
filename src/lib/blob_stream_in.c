...
void blobStreamInInit(BlobStreamIn *self, struct ImprintAllocator *memory,
                      ...
                      size_t octetCount, size_t fixedChunkSize, Clog log) {
  self->log = log;
  self->blob = IMPRINT_ALLOC((ImprintAllocator *)blobAllocator, octetCount,
                             "blob stream in payload");
  self->octetCount = octetCount;
  self->fixedChunkSize = fixedChunkSize;
  self->isComplete = 0;
  self->blobAllocator = blobAllocator;
  int chunkCount =
      (octetCount + self->fixedChunkSize - 1) / self->fixedChunkSize;
  bitArrayInit(&self->bitArray, memory, chunkCount);

  CLOG_C_VERBOSE(&self->log, "initialize. Expecting %zu octets",
               self->octetCount)
}
...
void blobStreamInSetChunk(BlobStreamIn *self, BlobStreamChunkId chunkId,
  ...
  CLOG_C_VERBOSE(&self->log, "setChunk chunkId: %hu octetCount: %zu", chunkId,
               octetCount)
  ...
  if (chunkId == self->bitArray.bitCount - 1) {
    ...
    if (octetCount != expectedLastChunkSize) {
      CLOG_C_ERROR(&self->log, "last chunk size must exactly. %zu vs %zu", octetCount,
                 expectedLastChunkSize)
    }
  } else {
    if (octetCount != self->fixedChunkSize) {
      CLOG_C_ERROR(&self->log, "chunk size must be equal to fixed chunk size. %zu vs %zu",
                 octetCount, self->fixedChunkSize)
    }
  }
